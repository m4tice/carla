...
import numpy as np
from c_utils import training_util as tu
...
# == INITIALIZATION =========
def initialization(town_dic, main_path):
    map_name = town_dic['name']
    weather_type = town_dic['weather']
    current_town = town_dic['dir_name']

    img_dir = os.path.join(main_path, "collected_data/camera_rgb/{}/IMG".format(current_town))
    csv_file = os.path.join(main_path, "collected_data/camera_rgb/{}/driving_log.csv".format(current_town))
    lidar_dir = os.path.join(main_path, "collected_data/lidar/{}".format(current_town))
    gnss_csv = os.path.join(main_path, "collected_data/{}/gnss_data.csv".format(current_town))
    imu_csv = os.path.join(main_path, "collected_data/{}/imu_data.csv".format(current_town))

    gps_intersection_csv = os.path.join(main_path, "database/{}/{}-gps-intersections.csv".format(current_town, current_town))
    waypoints_csv = os.path.join(main_path, "database/{}/{}-waypoints.csv".format(current_town, current_town))
    spawn_csv = os.path.join(main_path, "database/{}/{}-spawn-locations.csv".format(current_town, current_town))
    carla_intersection_csv = os.path.join(main_path, "database/{}/{}-intersections.csv".format(current_town, current_town))
    ...
    return map_name, weather_type, current_town, img_dir, csv_file, lidar_dir, gnss_csv, imu_csv, gps_intersection_csv, waypoints_csv, spawn_csv, carla_intersection_csv
...
def spawn_vehicle(world, blueprint_library, model, spawn_number=None):
...
def camera_drive_global_function_01(data, control_unit, min_spd=2, max_spd=4):
    control_unit.level_one(data, min_spd, max_spd)


def camera_drive_global_function_02(data, control_unit, gnss_data, world, weathers, dynamic_weather=False, min_spd=2, max_spd=4):
    if len(gnss_data) > 0:
        control_unit.auto_switch_02(data, min_spd, max_spd, gnss_data, camera=False)
...
def camera_drive_global_function_03(data, control_unit, lidar_data, gnss_data, world, weathers, dynamic_weather=False, min_spd=2, max_spd=4):
    if len(gnss_data) > 0 and len(lidar_data) > 0:
        control_unit.auto_switch_03(data, min_spd, max_spd, lidar_data, gnss_data, camera=False)
...
# -- Export lidar data ---
def export_lidar_csv(csv_file, features, loc_x, loc_y, theta):
    ...
    if os.path.isfile(csv_file):
        with open(csv_file, 'a', newline='') as file:
            ...
            for item0, item1, item2, item3 in features:
                writer.writerow([item0, item1, item2, item3, loc_x, loc_y, theta])
    else:
        with open(csv_file, 'w', newline='') as file:
            writer = csv.writer(file)
            for item0, item1, item2, item3 in features:
...
def display_lidar(point_cloud, vehicle, set_dir, lidar_range):
    ...
    try:
        ...
        export_lidar_csv(file, points, loc_x, loc_y, rad)
